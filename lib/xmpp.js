...
xmpp.setLogger = function(logger) {
	xmpp.log = logger;
}
...
exports.Connection.prototype = {
	connect: function (jid, pass, callback)	{
	    ...
	    this.socket
		.on("error", recontext(this, conn._socket_error))
	...
	_socket_error: function(e) {
		this._setStatus(xmpp.Status.ERROR, e);
		this.error(e);
	},
  ...
  // Logging
	...
	if (level === xmpp.LogLevel.DEBUG)
 	    xmpp.log.debug(prefix + "DEBUG: " + message);
	else if (level === xmpp.LogLevel.INFO)
		xmpp.log.info(prefix + "INFO: " + message);
	else if (level === xmpp.LogLevel.WARN)
		xmpp.log.warn(prefix + "WARN: " + message);
	else if (level === xmpp.LogLevel.ERROR)
		xmpp.log.error(prefix + message);
